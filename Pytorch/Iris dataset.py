...
def evaluate_untrained_model(model, data, criterion = torch.nn.CrossEntropyLoss()):
    ...
    loss=criterion(y_raw_pred,y_test.long())       # calculate loss
...
def evaluate_model(model, data, optimizer, lr = 0.1, criterion = torch.nn.CrossEntropyLoss(), 
    ...
    for epoch in range(number_of_epochs): 
        ...
        loss=criterion(y_prediction,y_train.long()) # calculate losses
        ...
        train_log_loss = criterion(model(x_train), y_train.long()).detach().numpy()
        test_log_loss = criterion(model(x_test), y_test.long()).detach().numpy()
...
def evaluate_with_model_regularization(model, data, optimizer, criterion = torch.nn.CrossEntropyLoss(), 
    ...
    for epoch in range(number_of_epochs): 
        ...
        if reg_type == 'l1':
            ...
            l_penalty = loss + reg_lambda * l1_norm
        if reg_type == 'l2':
            l2_norm = sum(torch.linalg.norm(p, 2) for p in model.parameters())
            l_penalty = loss + reg_lambda * l2_norm
...
def learning_rate_iter_explore(model, data, optimizer, lr_range = [0.99],
    ...
    for i, lr in enumerate(lr_range):
        ...
        for j in range(len(cur_model.history['train_loss'])):
            if abs(train_log_loss - cur_model.history['train_loss'][j]) < 100:
                best_iters = j
        y = lr
    # YOUR CODE HERE    
    # ??????????????
    highest_accuracy = max(resultsDict['test_accuracy'])
    
    #print(resultsList[0][1])
    for x in range(0,len(resultsList)):
        try:
            highest_index = resultsList[x].index(highest_accuracy)
            break
        except:
            pass
    pos = x
    best_lr = float(resultsDict['learning_rates'][pos])
    returnDict  = {'train_log_loss':resultsDict['train_log_loss'], 'test_log_loss':resultsDict['test_log_loss'], 'test_accuracy':resultsDict['test_accuracy'][pos], 'learning_rates':[], 'iters_for_convergence': int, 'best_lr': float} 
    
    returnDict['learning_rates'].append(float(lr))
    returnDict['iters_for_convergence'] = best_iters
    returnDict['best_lr'] = best_lr
    # make this have the same key-value structure as resultsDict plus an 'iters_for_convergence': scalar int and 'best_lr': scalar float
...
def main():
    ...
    # Func3_call 
    # ---|
    evaluate_error_metrics(model, data, optimizer)
    ...
    # Func5_call 
    learning_rate_iter_explore(model, data, optimizer)
